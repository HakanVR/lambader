...
class JxHeaderAccountFacebookLinkModuleFrontController extends ModuleFrontController
{
    public function initContent()
    {
        parent::initContent();

        $back = $this->context->link->getModuleLink('jxheaderaccount', 'facebooklink', array(), true, $this->context->language->id);

        if (!$this->context->customer->isLogged()) {
            Tools::redirect('index.php?controller=authentication&back='.urlencode($back));
        }

        $facebookid = Configuration::get('JXHEADERACCOUNT_FAPPID');
        $facebookkey = Configuration::get('JXHEADERACCOUNT_FAPPSECRET');
        ...
        $this->setTemplate('module:jxheaderaccount/views/templates/front/facebooklink.tpl');
    }
}
