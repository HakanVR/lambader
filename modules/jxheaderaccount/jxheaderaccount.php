...
class Jxheaderaccount extends Module implements WidgetInterface
{
    ...
    {
        if (!$this->facebook_compatibility) {
            $this->warning = $this->l('Jetimpex Social Login(Facebook) requires PHP version 5.4 or higher. Facebook login will be unavailable.');
        }
        if (!function_exists('curl_init')) {
            $this->facebook_compatibility = false;
            $this->warning = $this->l('Jetimpex Social Login(Facebook) need the CURL PHP extension. Facebook login will be unavailable.');
        }
        if (!function_exists('json_decode')) {
            $this->facebook_compatibility = false;
            $this->warning = $this->l('Jetimpex Social Login(Facebook) need the JSON PHP extension. Facebook login will be unavailable.');
        }
        if (!function_exists('hash_hmac')) {
            $this->facebook_compatibility = false;
            $this->warning = $this->l('Jetimpex Social Login(Facebook) need the HMAC Hash (hash_hmac) PHP extension. Facebook login will be unavailable.');
        }
    }
    ...
    {
        $this->validateFbFields($options['JXHEADERACCOUNT_FSTATUS'], $options['JXHEADERACCOUNT_FAPPID'], $options['JXHEADERACCOUNT_FAPPSECRET']);
        $this->validateGoogleFields($options['JXHEADERACCOUNT_GSTATUS'], $options['JXHEADERACCOUNT_GAPPID'], $options['JXHEADERACCOUNT_GAPPSECRET'], $options['JXHEADERACCOUNT_GREDIRECT']);
        $this->validateVkFields($options['JXHEADERACCOUNT_VKSTATUS'], $options['JXHEADERACCOUNT_VKAPPID'], $options['JXHEADERACCOUNT_VKAPPSECRET'], $options['JXHEADERACCOUNT_VKREDIRECT']);
    ...
    public function processImageUpload($FILES)
    {
        if (isset($FILES['JXHEADERACCOUNT_AVATAR']) && isset($FILES['JXHEADERACCOUNT_AVATAR']['tmp_name']) && !empty($FILES['JXHEADERACCOUNT_AVATAR']['tmp_name'])) {
            if ($error = ImageManager::validateUpload($FILES['JXHEADERACCOUNT_AVATAR'], 4000000)) {
                return $this->displayError($this->l('Invalid image'));
            } else {
                $ext = Tools::substr($FILES['JXHEADERACCOUNT_AVATAR']['name'], strrpos($FILES['JXHEADERACCOUNT_AVATAR']['name'], '.') + 1);
