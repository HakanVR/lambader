...
include_once _PS_MODULE_DIR_ . 'jxmediaparallax/classes/JXMediaParallaxItems.php';
include_once _PS_MODULE_DIR_ . 'jxmediaparallax/classes/JXMediaParallaxLayouts.php';

class Jxmediaparallax extends Module
{
    protected $default_types;
    protected $languages;
    public function __construct()
    {
        $this->name = 'jxmediaparallax';
        $this->tab = 'front_office_features';
        $this->version = '2.0.0';
        $this->author = 'Jetimpex';
        ...
        $this->displayName = $this->l('JX Media Paralax');
    ...
    protected function getPageContent()
    {
        if (Shop::getContext() == Shop::CONTEXT_GROUP || Shop::getContext() == Shop::CONTEXT_ALL) {
        ...
        } elseif ((bool)Tools::isSubmit('addjxmediaparallax') || (bool)Tools::isSubmit('updatejxmediaparallax')) {
            return $this->renderParallaxForm();
        } elseif ((bool)Tools::isSubmit('savejxmediaparallax')) {
        ...
        } elseif ((bool)Tools::isSubmit('deletejxmediaparallax')) {
            $id_item = Tools::getValue('id_item');
            $this->deleteItem($id_item);
            return $this->renderParallaxList();
        } elseif ((bool)Tools::isSubmit('deletejxmediaparallaxlayouts')) {
            $id_layout = Tools::getValue('id_layout');
            $this->deleteLayout($id_layout);
            return $this->renderLayoutsList();
        } elseif ((bool)Tools::isSubmit('savejxmediaparallaxlayout')) {
            ...
            $layout = new JXMediaParallaxLayouts(Tools::getValue('id_layout'));
            if ($layout->toggleStatus()) {
                $this->_confirmations[] = $this->l('Layout status updated');
            }
            $this->clearCache();
            return $this->renderLayoutsList();
        } elseif (Tools::getIsset('viewjxmediaparallax') || Tools::getIsset('jxmediaparallaxlayouts')) {
            return $this->renderLayoutsList();
        } elseif ((bool)Tools::isSubmit('addjxmediaparallaxlayout') || (bool)Tools::isSubmit('updatejxmediaparallaxlayouts')) {
            return $this->renderLayoutForm();
        } elseif (Tools::isSubmit('itemstatus')) {
            $item = new JXMediaParallaxItems(Tools::getValue('id_item'));
    ...
    protected function renderParallaxList()
    {
        ...
        $helper->toolbar_btn['new'] = array(
            'href' => AdminController::$currentIndex.'&configure='.$this->name.'&addjxmediaparallax&token='.Tools::getAdminTokenLite('AdminModules') . '&id_shop='.$this->id_shop,
            'desc' => $this->l('Add new')
        );
    ...
    {
        return JXMediaParallaxItems::getItems($this->id_shop);
    }
    ...
    protected function getConfigParallaxFormValues()
    {
        ...
        if (!$id_item =  Tools::getValue('id_item')) {
            $item = new JXMediaParallaxItems();
        } else {
            $item = new JXMediaParallaxItems($id_item);
        }
    ...
    protected function renderLayoutsList()
    {
        ...
        $helper->toolbar_btn['new'] = array(
            'href' => AdminController::$currentIndex.'&configure='.$this->name.'&addjxmediaparallaxlayout&id_item='.Tools::getValue('id_item').'&token='.Tools::getAdminTokenLite('AdminModules').'&id_shop='.$this->id_shop,
            'desc' => $this->l('Add new')
        );
    ...
    protected function getConfigLayoutsListValues()
    {
        ...
        $layouts = JXMediaParallaxLayouts::getLayouts($id_patent);
    ...
    protected function renderLayoutForm()
    {
        ...
        $helper->table = $this->name . 'layouts';
        $helper->identifier = 'id_layout';
        $helper->currentIndex = AdminController::$currentIndex . '&configure=' . $this->name .'&viewjxmediaparallax&id_item=' . Tools::getValue('id_item') . '&id_shop='.$this->id_shop;
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->tpl_vars = array(
        ...
        return array(
            'form' => array(
                ...
                'submit' => array(
                    'title' => $this->l('Save'),
                    'type' => 'submit',
                    'name' => 'savejxmediaparallaxlayout'
                ),
                'buttons' => array(
                    array(
                        'href' => AdminController::$currentIndex.'&configure='.$this->name.'&viewjxmediaparallax&id_item='.Tools::getValue('id_item').'&token='.Tools::getAdminTokenLite('AdminModules').'&id_shop='.$this->id_shop,
                        'title' => $this->l('Cancle'),
                        'icon' => 'process-icon-cancel'
                    )
                )
    ...
    protected function getConfigLayoutFormValues()
    {
        ...
        if (!$id_layout =  Tools::getValue('id_layout')) {
            $item = new JXMediaParallaxLayouts();
        } else {
            $item = new JXMediaParallaxLayouts($id_layout);
        }
    ...
    {
        if (!$id_layout =  Tools::getValue('id_layout')) {
            $layout = new JXMediaParallaxLayouts();
        } else {
            $layout = new JXMediaParallaxLayouts($id_layout);
        }
        $type = Tools::getValue('type');
    ...
    {
        $items = JXMediaParallaxItems::getItems($this->id_shop, true);
        foreach ($items as $key => $item) {
            if ($mobile) {
                $layout = $this->getBackgroundLayout(JXMediaParallaxLayouts::getLayouts($item['id_item'], true));
                if (is_array($layout)) {
                    $items[$key] = array_merge($layout, $items[$key]);
                }
            } else {
                $layouts = JXMediaParallaxLayouts::getLayouts($item['id_item'], true);
    ...
    {
        $items = JXMediaParallaxItems::getItems($this->id_shop);
        foreach ($items as $item) {
            JXMediaParallaxItems::addLang($id_lang, $item['id_item']);
            $layouts = JXMediaParallaxLayouts::getLayouts($item['id_item']);
            foreach ($layouts as $layout) {
                JXMediaParallaxLayouts::addLang($id_lang, $layout['id_layout']);
            }
        }
    }
    ...
    public function hookDisplayBackOfficeHeader()
    {
        if (Tools::getValue('configure') == $this->name) {
            ...
            $this->context->controller->addJS($this->_path.'views/js/jxmediaparallax_admin.js');
            $this->context->controller->addCSS($this->_path.'views/css/jxmediaparallax_admin.css');
        }
    }
    ...
    {
        $this->context->controller->registerJavascript('module-jxmediaparallax-rd', 'modules/' .$this->name. '/views/js/jquery.rd-parallax.min.js');
        $this->context->controller->registerJavascript('module-jxmediaparallax-yb', 'modules/' .$this->name. '/views/js/jquery.youtubebackground.js');
        $this->context->controller->registerJavascript('module-jxmediaparallax-vide', 'modules/' .$this->name. '/views/js/jquery.vide.min.js');
        $this->context->controller->registerJavascript('module-jxmediaparallax', 'modules/' .$this->name. '/views/js/jxmediaparallax.js');
        $this->context->controller->registerStylesheet('module-jxmediaparallax', 'modules/' .$this->name. '/views/css/jxmediaparallax.css');
        $this->context->controller->registerStylesheet('module-jxmediaparallax-rd', 'modules/' .$this->name. '/views/css/rd-parallax.css');
    }

    public function hookDisplayBeforeBodyClosingTag()
    {
        if ($this->context->isMobile() || $this->context->isTablet()) {
            if (!$this->isCached('layout-mobile.tpl', $this->getCacheId('jxmediaparallax'))) {
                $items = $this->getItems(true);
                $this->context->smarty->assign(array('items' => $items, 'base_url' => __PS_BASE_URI__));
            }

            return $this->display(__FILE__, 'views/templates/hook/layout-mobile.tpl', $this->getCacheId('jxmediaparallax'));
        } else {
            if (!$this->isCached('layouts.tpl', $this->getCacheId('jxmediaparallax'))) {
                $items = $this->getItems();
                $this->context->smarty->assign(array('items' => $items, 'base_url' => __PS_BASE_URI__));
            }

            return $this->display(__FILE__, 'views/templates/hook/layouts.tpl', $this->getCacheId('jxmediaparallax'));
        }
    }
