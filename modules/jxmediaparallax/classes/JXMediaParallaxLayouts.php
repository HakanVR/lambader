...
class JXMediaParallaxLayouts extends ObjectModel
{
    public $id_layout;
    public $id_parent;
    public $fade;
    ...
    {
        $sql = 'SELECT *
                FROM ' . _DB_PREFIX_ . 'jxmediaparallax_layouts jxmp
                JOIN ' . _DB_PREFIX_ . 'jxmediaparallax_layouts_lang jxmpl
                ON jxmp.id_layout = jxmpl.id_layout
                AND jxmpl.id_lang = '. Context::getContext()->language->id.'
                AND jxmp.id_parent = '. $id_parent;
        if ($only_active) {
            $sql .= ' AND `active` = 1';
        }
    ...
    {
        $layouts = JXMediaParallaxLayouts::getLayouts($id_parent);
        foreach ($layouts as $layout) {
            if (!JXMediaParallaxLayouts::deleteLang($layout['id_layout'])) {
                return false;
            }
        }

        $where = '`id_parent` ='. $id_parent;
        $table = _DB_PREFIX_ . 'jxmediaparallax_layouts';
    ...
    public static function deleteLang($id_layout)
    {
        ...
        $table = _DB_PREFIX_ . 'jxmediaparallax_layouts_lang';
    ...
    {
        if (!JXMediaParallaxLayouts::checkLang($id_lang, $id_layout)) {
            $table = 'jxmediaparallax_layouts_lang';
    ...
    {
        $sql = 'SELECT *
                FROM ' . _DB_PREFIX_ . 'jxmediaparallax_layouts_lang 
