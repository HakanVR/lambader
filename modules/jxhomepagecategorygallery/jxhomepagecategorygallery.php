...
include_once(_PS_MODULE_DIR_.'jxhomepagecategorygallery/classes/HomepageCategoryGallery.php');

use PrestaShop\PrestaShop\Adapter\ObjectPresenter;
use PrestaShop\PrestaShop\Adapter\Image\ImageRetriever;

class Jxhomepagecategorygallery extends Module
{
    private $list = array();
    ...
    {
        $this->name = 'jxhomepagecategorygallery';
        ...
        parent::__construct();
        $this->displayName = $this->l('JX Homepage Category Gallery');
    ...
    public function install()
    {
        ...
        return parent::install()
            ...
            && Configuration::updateValue('JX_CATEGORY_GALLERY_ENABLE', true);
    ...
    public function uninstall()
    {
        ...
        Configuration::deleteByName('JX_CATEGORY_GALLERY_ENABLE');
    ...
    public function getContent()
    {
        $output = '';
        $check = '';
        if (((bool)Tools::isSubmit('submitJxhomepagecategorygalleryModule')) == true) {
    ...
    */
        ...
        $fields_form = array(
            'form' => array(
                ...
                'input' => array(
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Display gallery'),
                        'name' => 'JX_CATEGORY_GALLERY_ENABLE',
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
    ...
    {
        return array(
            'JX_CATEGORY_GALLERY_ENABLE' => Tools::getValue('JX_CATEGORY_GALLERY_ENABLE', Configuration::get('JX_CATEGORY_GALLERY_ENABLE')),
        );
    }
    ...
    */
    protected function renderForm($id_item = 0)
    {
        ...
        $helper->identifier = $this->identifier;
        $helper->submit_action = 'submitJxhomepagecategorygalleryModule';
        $helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false)
            .'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name;
        $helper->token = Tools::getAdminTokenLite('AdminModules');
...
;
    ...
    */
    private function getItemsList($active = false)
    {
        ...
        $sql = 'SELECT thcg.*, cl.`name`, thcgl.`content`
                FROM '._DB_PREFIX_.'jxhomepagecategorygallery thcg
                LEFT JOIN '._DB_PREFIX_.'jxhomepagecategorygallery_shop thcgs
                ON (thcg.`id_item` = thcgs.`id_item`)
                LEFT JOIN '._DB_PREFIX_.'jxhomepagecategorygallery_lang thcgl
    ...
    */
    protected function getRelatedItems($id_category)
    {
        ...
        $sql = 'SELECT `id_item`
                FROM '._DB_PREFIX_.'jxhomepagecategorygallery
    ...
    */
    ...
    public function hookHeader()
    {
        ...
        if (Configuration::get('JX_CATEGORY_GALLERY_ENABLE')) {
            $this->context->controller->addJQueryPlugin('scrollTo');
            $this->context->controller->registerJavascript('module-jxhomepagecategorygallery', 'modules/' . $this->name . '/views/js/jxhomepagecategorygallery.js');
        }

        $this->context->controller->registerStylesheet('module-jxhomepagecategorygallery', 'modules/'.$this->name.'/views/css/jxhomepagecategorygallery.css');
    }
    ...
    */
    ...
    public function hookDisplayHome()
    {
        ...
        $image = new ImageRetriever($this->context->link);
        $result = array();
        $items = $this->getItemsList(true);
        if ($items) {
            foreach ($items as $key => $item) {
                $category = new Category((int)$item['id_category'], $this->context->language->id);
                ...
                $result[$key]['category'] = $presenter->present($category);
                $result[$key]['image'] = $image->getImage(
                    $category,
                    $category->id_image
                );
            }
        ...
        $this->context->smarty->assign('display_gallery', Configuration::get('JX_CATEGORY_GALLERY_ENABLE'));

        return $this->display($this->_path, '/views/templates/hook/jxhomepagecategorygallery.tpl');
    }
}
