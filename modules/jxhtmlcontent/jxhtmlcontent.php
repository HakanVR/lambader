...
class jxhtmlcontent extends Module
{
	...
	public function __construct()
	{
		...
		parent::__construct();
		$this->displayName = $this->l('JX HTML Content');
	...
	public function createAjaxController()
	{
		...
		if (is_array($languages))
			foreach ($languages as $language)
				$tab->name[$language['id_lang']] = 'jxhtmlcontent';
	...
	{
		return (
			Db::getInstance()->Execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'jxhtmlcontent`') &&
			Db::getInstance()->Execute('
			CREATE TABLE `'._DB_PREFIX_.'jxhtmlcontent` (
	...
	{
		$images = array();
		if (count(Db::getInstance()->executeS('SHOW TABLES LIKE \''._DB_PREFIX_.'jxhtmlcontent\'')))
			$images = Db::getInstance()->executeS('SELECT image FROM `'._DB_PREFIX_.'jxhtmlcontent`');
		foreach ($images as $image)
			$this->deleteImage($image['image']);

		if (!Db::getInstance()->Execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'jxhtmlcontent`') || !$this->_removeAjaxContoller() || !parent::uninstall())
			return false;
	...
	public function hookDisplayBackOfficeHeader()
	{
		...
		$this->context->controller->addCSS($this->_path.'css/jxhtmlcontent_admin.css');
		$this->context->controller->addJquery();
		$this->context->controller->addJS($this->_path.'js/jxhtmlcontent_admin.js');
	}

	public function hookDisplayHeader()
	{
		$this->context->controller->registerStylesheet('module-jxhtmlcontent', 'modules/' .$this->name. '/css/jxhtmlcontent.css');
	}

	public function hookdisplayNav1()
	{
		$this->context->smarty->assign(array(
			'htmlitems' => $this->getItemsFromHook('displayNav1'),
			'hook' => 'displayNav1'
		));
	...
	protected function getItemsFromHook($hook)
	{
		...
		SELECT *
		FROM `'._DB_PREFIX_.'jxhtmlcontent`
	...
	protected function removeItem()
	{
		...
		if ($image = Db::getInstance()->getValue('SELECT image FROM `'._DB_PREFIX_.'jxhtmlcontent` WHERE id_item = '.(int)$id_item))
			$this->deleteImage($image);

		Db::getInstance()->delete(_DB_PREFIX_.'jxhtmlcontent', 'id_item = '.(int)$id_item);

		if (Db::getInstance()->Affected_Rows() == 1)
		{
			Db::getInstance()->execute('
				UPDATE `'._DB_PREFIX_.'jxhtmlcontent` 
	...
	protected function updateItem()
	{
		...
		{
			if ($old_image = Db::getInstance()->getValue('SELECT image FROM `'._DB_PREFIX_.'jxhtmlcontent` WHERE id_item = '.(int)$id_item))
		...
		if (!Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'jxhtmlcontent` SET 
					title = \''.pSQL($title).'\',
		...
		{
			if ($image = Db::getInstance()->getValue('SELECT image FROM `'._DB_PREFIX_.'jxhtmlcontent` WHERE id_item = '.(int)Tools::getValue('item_id')))
				$this->deleteImage($image);
	...
	protected function addItem()
	{
		...
		if (!Db::getInstance()->Execute('
			INSERT INTO `'._DB_PREFIX_.'jxhtmlcontent` ( 
		...
		foreach ($this->languages as $language)
		{
			...
			foreach ($hooks[$language['id_lang']] as $hook)
				$items[$language['id_lang']][$hook] = Db::getInstance()->ExecuteS('
					SELECT * FROM `'._DB_PREFIX_.'jxhtmlcontent` 
