...
{if isset($items) && $items}
	<div class="jxhomepagecategorygallery-block">
        <ul id="jxhomepagecategorygallery" class="jxhomepagecategorygallery row">
            {foreach from=$items item=item name=slide}
                <li id="jxhomepagecategorygallery-item-{$smarty.foreach.slide.iteration}" class="item-{$item.id_item} col-4 item{if isset($item.specific_class) && $item.specific_class} {$item.specific_class}{/if}">
