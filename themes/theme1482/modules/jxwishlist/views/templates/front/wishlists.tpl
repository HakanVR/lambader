  ...
  <section id="main">
    <div id="mywishlists">
      ...
      <header class="page-header">
        <h1 class="products-section-title">{l s='My wishlists' mod='jxwishlist'}</h1>
      </header>
          ...
          {foreach from=$wishlists item=wishlist name=wishlist}
            <li data-wishlist-id="{$wishlist.id_wishlist|intval}" id="wishlist_{$wishlist.id_wishlist|intval}" data-wishlist-name="{$wishlist.name|truncate:22:'...'|escape:'html':'UTF-8'}">
              ...
              <div class="clearfix wishlist-row-bottom">
                <a target="_blank" href="{$link->getModuleLink('jxwishlist', 'wishlist', ['token' => $wishlist.token])|escape:'htmlall':'UTF-8'}" class="btn btn-view-wishlist button btn-primary">
                  {l s='View wishlist' mod='jxwishlist'}
                </a>
                {if $products}
                  <button type="button" id="add-new-layout" class="btn btn-secondary"><span>{l s='Share' mod='jxwishlist'}</span></button>
                {/if}
